...
impl FromStr for Boot {
    ...
    fn from_str(s: &str) -> Result<Self> {
        let (active, zone) = s
            .split_once(' ')
            .wrap_err_with(|| format!("Bad format for '{s}'"))?;
        ...
        let (x, y, z) = s
            ...
            .map(|coord| -> Result<Range<i32>> {
                let c = coord
                    ...
                    .wrap_err_with(|| format!("Missing x=, y= or z= prefix: '{coord}'"))?;

                let (from, to) = c
                    .splitn(2, "..")
                    .map(|n| n.parse().wrap_err_with(|| format!("In '{c}'")))
                    .collect_tuple::<(Result<i32>, Result<i32>)>()
                    .wrap_err_with(|| format!("Missing '..' range delimiter in '{c}'"))?;

                let (from, to) = (from?, to?);
                Ok(from.min(to)..(from.max(to) + 1))
            })
            .collect_tuple::<(_, _, _)>()
            .wrap_err_with(|| format!("Missing X, Y or Z in '{s}'"))?;
